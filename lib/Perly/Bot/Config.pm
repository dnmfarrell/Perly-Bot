...
BEGIN {
  ...
  sub new (
    $class,
    $file = catfile( $ENV{HOME}, '.perlybot', 'config' ),
    $overrides = {} )
  {
...
sub load_config (
  $self,
  $file =
    do { 'config.yml' ? 'config.yml' : "$ENV{HOME}/.perly_bot/config.yml" }
  )
{
...
sub reddit ( $self ) {
  $self->media->{'Perly::Bot::Media::Reddit'}{params} // {};
}
...
sub twitter ( $self ) {
  $self->media->{'Perly::Bot::Media::Twitter'}{params} // {};
}
